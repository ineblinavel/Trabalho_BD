...
async function verSlots(idAgenda) {
    ...
    try {
        ...
        if (slots.length === 0) {
            container.innerHTML = '<small class="text-muted fst-italic">Nenhum horário livre neste plantão.</small>';
            return;
        }
...
async function carregarConsultas(crm) {
    ...
    try {
        ...
        consultas.forEach(c => {
            ...
            let statusBadge = '';
            if(c.status === 'C') statusBadge = '<span class="badge bg-success ms-2">Concluída</span>';
            else if(c.status === 'A') statusBadge = '<span class="badge bg-warning text-dark ms-2">Agendada</span>';

            let buttons = '';
            if (c.status !== 'C') {
                buttons += `
                    <button class="btn btn-sm btn-danger me-2" onclick="abrirModalAtendimento(${c.id_consulta}, '${c.nome_paciente}', '${c.cpf || ''}')">
...
async function carregarTiposExame() {
    try {
        ...
        select.innerHTML = '<option value="" selected disabled>Selecione um exame...</option>';
        tipos.forEach(t => {
...
async function carregarPacientesSelect() {
    try {
        ...
        select.innerHTML = '<option value="" selected disabled>Selecione um paciente...</option>';
        pacientes.forEach(p => {
...
async function carregarMedicosSelect() {
    try {
        ...
        select.innerHTML = '<option value="" selected disabled>Selecione um médico...</option>';
        medicos.forEach(m => {
...
async function abrirModalAtendimento(idConsulta, nomePaciente, cpf) {
    ...
    document.getElementById('atend-paciente-cpf').textContent = `CPF: ${cpf || 'Não informado'}`;
...
async function finalizarAtendimento() {
    ...
    if (!diagnostico) return alert("Por favor, descreva o diagnóstico antes de finalizar.");
